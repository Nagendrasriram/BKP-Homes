        ...
        >
          {COLLECTIONS.map((item, idx) => (
            <motion.div
              ...
              className={`${item.size} group relative h-[380px] rounded-2xl overflow-hidden border border-white/5`}
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 z-20 flex flex-col justify-end h-full">
                <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500 ease-out">
                  ...
                  <h3 className="font-serif text-2xl font-light text-luxury-cream mb-2">
                    {item.title}
                  </h3>
                  
                  {/* Expandable description */}
                  <p className="text-xs text-luxury-beige/0 group-hover:text-luxury-beige/80 transition-colors duration-300 font-light leading-relaxed max-w-sm mb-4">
                    {item.description}
                  </p>
